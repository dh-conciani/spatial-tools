...
ref_proj  <- crs(eecl)                                          # Extract projection reference
proj_list <- lapply (rs_list, reproject_raster)                 # Reproject rasters to reference projection
rm (rs_list, ref_proj)

# 2. Reclassify rasters to binary images (1= burned area; 0= unburned area)
rcl_list <- lapply (proj_list, reclass_nbr)             # Perform reclass and make binary raster's
rm (proj_list, m, rclmat)   

# 3. Extract only interest areas 
eecl_list    <- lapply (rcl_list, eecl_mask)    # Extract to EEcl
eei_list     <- lapply (rcl_list, eei_mask)     # Extract to EEI
buffer_list  <- lapply (rcl_list, buffer_mask)  # Extract to Buffer
rm (rcl_list)

# 4. Calc burned area (in hectares) and percentage of burned area
# a. Ecological Station of Itirapina
local      = rep("EEcl",length(base_lists))                                            # Local name              
area       = rep(2678, length(base_lists))                                             # Area of local
ba_eecl   <- lapply (eecl_list, area_fun)                                              # List burned area in EEcl
             ...
             df_eecl    = data.frame (years, months, burned_area, local, area)         # Structure data frame
...
local      = rep("EEI",length(base_lists))                                             # Local name              
area       = rep(3131, length(base_lists))                                             # Area of local
ba_eei   <- lapply (eei_list, area_fun)                                                # List burned area in EEcl
             burned_area= as.matrix (ba_eei)                                           # Make a matrix
             burned_area= as.numeric (burned_area)                                     # Convert to numeric variable
             df_eei    = data.frame (years, months, burned_area, local, area)          # Structure data frame
...
local      = rep("Buffer",length(base_lists))                                               # Local name              
area       = rep(42340, length(base_lists))                                                 # Area of local
ba_buffer   <- lapply (buffer_list, area_fun)                                               # List burned area in EEcl
             burned_area= as.matrix (ba_buffer)                                             # Make a matrix
             burned_area= as.numeric (burned_area)                                          # Convert to numeric variable
             df_buffer    = data.frame (years, months, burned_area, local, area)            # Structure data frame
